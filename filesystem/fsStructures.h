...
struct directoryEntry {
    ...
    unsigned long long fileIndexLocation;
    ...
    unsigned long int dateAccessed;
...
/*
Total bytes used by volume control block: 392 bits - 49 bytes
...
struct volumeControlBlock {
    ...
    // Always stored in block 0 in the LBA
    // Can be changed if the LBA block becomes corrupt, potentially up to block 255
    // 8 bits [0, 255]
    unsigned char rootLocation;
    ...
    // Clusterity: No clusters will be implemented in this file system for now
};


/*
 Description: Keeps track of the free blocks in the LBA
 Total bytes used by free space information block: 128 bits PLUS size of bitArray[]. Size of bitArray[] will depend on how many LBA blocks there are, since each LBA block has 1 bit corresponding to it
*/
struct freeSpaceInformation {
    // Not sure if signature required for bitmap. How could the struct become corrupted? Maybe use a hash for comparing?
    // Ensures that our bitmap has not been corrupted. This will be a random number we can later check against
    // 64 bits [0, 18,446,744,073,709,551,615]
...
/*
 ...
 Total bytes used by file index information block: 64 bits PLUS size of fileIndexLocation[]. Size of fileIndexLocation[] will depend on how many extents the file has
*/
struct fileIndexInformation {
    ...
    // 64 bits [0, 18,446,744,073,709,551,615]
    unsigned long long nextFileIndexLocation;
    
    // An array of block numbers in the LBA files are stored. Max size is 63 elements before LBA block is full
    // 64 bits [0, 18,446,744,073,709,551,615] for each element in the array
    unsigned long long fileIndexLocation[];
};
